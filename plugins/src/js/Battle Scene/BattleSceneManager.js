...
BattleSceneManager.prototype.init = async function(attachControl){
	...
	if(!this._initialized){
		...
		this._effksContextMirror.init(this._glContext);
		this._effksContextBg.init(this._glContext);
		this._effksContextBgMirror.init(this._glContext);
		this._effksContextAttached.init(this._glContext);
...
BattleSceneManager.prototype.createModel = async function(name, path, position, flipX, animGroup, animName, scale, rotation){
	/*var dynamicBgInfo = this.createSpriterBg(name+"_3d", position, size || 10, 1, 0, flipX, null, useAlpha, null, width, height);
	dynamicBgInfo.renderer = new SpineManager();
	dynamicBgInfo.renderer.startAnimation(dynamicBgInfo, "img/SRWBattleScene/"+path, animName || "main");
	this._unitModelInfo.push(dynamicBgInfo);
	return dynamicBgInfo;*/
...
BattleSceneManager.prototype.createUnitModel = async function(name, path, position, flipX, animGroup, animName, scale, rotation){
	/*var dynamicBgInfo = this.createSpriterBg(name+"_3d", position, size || 10, 1, 0, flipX, null, useAlpha, null, width, height);
	dynamicBgInfo.renderer = new SpineManager();
	dynamicBgInfo.renderer.startAnimation(dynamicBgInfo, "img/SRWBattleScene/"+path, animName || "main");
	this._unitModelInfo.push(dynamicBgInfo);
			...
			if(texture){
				...
				if(refWidth){
					...
					if(texture.uOffsetAccumulator == null){
						texture.uOffsetAccumulator = texture.uOffset;
					}
...
BattleSceneManager.prototype.startScene = function(){
	...
	this._scene.onBeforeRenderObservable.add(() => {
		...
		this._barrierEffects.forEach(function(effekInfo){
			if(effekInfo.parent && effekInfo.handle){
				...
				effekInfo.handle.setShown(effekInfo.handle.isEnabled && effekInfo.referenceSprite.sprite.isEnabled());
			}
		});
		*/
		if(!_this._animsPaused){
	...
	this._scene.onAfterParticlesRenderingObservable.add(() => {//onAfterRenderObservable 
		...
		_this._effksContextBgMirror.setProjectionMatrix(_this._camera.getProjectionMatrix().m);
		_this._effksContextBgMirror.setCameraMatrix(BABYLON.Matrix.Invert(_this._camera.getWorldMatrix()).m);
	...
	var animationHandlers = {
		...
		next_phase: function(target, params){
			...
			if(_this._currentAnimatedAction.attacked && _this._currentAnimatedAction.attacked.type == "support defend"){
				_this.delayAnimationList(startTick + 27, 170);
				...
				_this._animationList[startTick + 150] = [
					{type: "fade_swipe", target: "", params: {time: 54}},
				];
				
				_this._animationList[startTick + 160] = [
					{type: "show_sprite", target: "active_main", params: {}},	
					{type: "enable_support_defender"},
				];				
				
				if(params.commands){
					_this._animationList[startTick + 161] = params.commands;						
				}
				_this._animationList[startTick + 162] = [{type: "updateBgMode", target: "active_target"}];
			} else {
				if(params.commands){
					...
					if(_this._currentAnimatedAction.hits){
						var additions = [];
						additions[startTick + 50] = [{type: "show_barrier", target: "active_target", params: {}}];	
						//if(_this._currentAnimatedAction.attacked.action.type == "defend"){
						additions[startTick + 50].push({type: "set_sprite_frame", target: "active_target", params: {name: "block"}});
						//}
...
BattleSceneManager.prototype.createScrollingBg = function(id, bgId, path, size, yOffset, zOffset, alphaIndex){
	...
	for(var i = 0; i < amount; i++){
		var bg = this.createBg(id + "_" + i, path, new BABYLON.Vector3(startX + (i * size.width), yOffset, zOffset), size, 1, null, true, false, true);
		bg.visibility = 0;
		bg.bgId = bgId;
